...
BEGIN
	...
	DECLARE cur_people CURSOR LOCAL FOR
		SELECT CustomerID, BirthDate, Documento, Responsavel, flag_documento
		FROM inserted 

    OPEN cur_people
    
	FETCH NEXT FROM cur_people into @CustomerID, @BirthDate, @Documento, @Responsavel, @flag_documento

    WHILE @@FETCH_STATUS = 0  
		BEGIN 
			IF (YEAR(GETDATE()) - YEAR(@BirthDate) < 18) 
			  BEGIN
				IF @Documento IS NULL AND @responsavel IS NULL
					BEGIN
